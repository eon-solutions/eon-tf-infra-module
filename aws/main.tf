...
# Usage:
#   module "eon_aws" {
#     source = "github.com/eon-solutions/eon-tf-infra"
#
...
resource "terraform_data" "reconnect_source_account" {
  count = var.enable_source_account && local.source_account_disconnected ? 1 : 0
...
resource "terraform_data" "reconnect_restore_account" {
  count = var.enable_restore_account && local.restore_account_disconnected ? 1 : 0
